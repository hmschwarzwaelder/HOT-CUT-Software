class SliderV {
  ...
  SliderV (int _x, int _y, int _w, int _h, color _c1, color _c2) {
    x = _x;
    y = _y;
    w = _w;
    h = _h;
  ...
  boolean overSlider(int xa, int ya, int xb, int yb, int xc, int yc) 
  {
  ...
  boolean over() 
  {
    if ( overSlider(p+x, y, p+x+8, y, p+x+4, y+h)) {
      return true;
    } else {
      return false;
    }
